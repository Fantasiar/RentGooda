(function ($) {
	$.Tupload = {
		...
		createHtml:function(defaults){
			...
			html += 	'<ul>';
				...
				for(var i=0;i<fileNum;i++){
					...
					html += 	'<img id="img_src'+i+'" class="upload_image" src="/pages/images/imgadd.png"/>';
					html += '</div>';
		...
		imgLoad:function(i,file){
			...
			$(r).load(function(){
				while(true){
					if($("#img_src"+i).attr("src") != "/pages/images/imgadd.png"){
						i++;
					}else{
						break;
					}
		...
		formUpload:function (i,fileObj,FileController){
            ...
            form.append('fineness',$('#fineness').val());
            form.append('description',$('#area').val());
            form.append('deposite',$('#deposite').val());
		...
		onDelete:function(i){
			...
			arrFile[i] = '';
			$("#img_src"+i).attr("src","/pages/images/imgadd.png");
			$("#uploading-tip"+i).hide();
			$.Tupload.setPhotoImg();
		}
	}
...
$("#fileImage").live('change',function(){
	...
	if(num < $.Tupload.fileNum+1){
		...
		for(var i=0;i<num;i++){
			for(var j=0;j<num;j++){
				if($("#img_src"+j).attr("src") == "/pages/images/imgadd.png"){
...
$("#submit").live('click',function(){
	...
	for(var i=0;i<arrFile.length;i++){
		...
		if(arrFile[i] != undefined){
			fileObj[j] = arrFile[i]; // js 获取文件对象
            j++;
		}

	}
    var FileController = $.Tupload.options.url; // 接收上传文件的后台地址
    $.Tupload.formUpload(j,fileObj,FileController);
